...
public:
    ...
    void Enqueue(vector_data_t *vdata)
    {
    ...
    // Dequeue will block here if there is no message in queue
    vector_data_t *Dequeue()
    {
...
// use Round-Robin method for load balancing on Distribution queue
static void coordinator_enqueue_distribution_queue(cmsg_t *cmsg, std::shared_ptr<subscriber_db_entry_t> SubEntry)
{
...
// it is being executed in the context of coordinator listener thread
void coordinator_accept_pubmsg_for_distribution_to_subcriber(cmsg_t *cmsg)
{
...
static void *coordinator_listen_distribution_queue(void *arg)
{
    vector_data_t *vdata;
    CoordDistQueue *dist_queue = static_cast<CoordDistQueue *>(arg);
    while ((vdata = dist_queue->Dequeue())) // if queue is empty, then Deququ will be blocked, so,while()can never be false
    {
...
//
void coordiantor_fork_distributon_threads()
{
    pthread_t *thread;
