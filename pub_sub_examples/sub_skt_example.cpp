...
// udp port no
#define SUB_SKT_UDP_PORT_NO 50001 // port number defined randomly?
uint16_t argument_port_no = 0;
static char buffer[1024];

void *sub_skt_example(void *arg)
{
    ...
    while (1)
    {
        ...
        char *tlv_value = tlv_buffer_get_particualr_tlv(tlv_buffer, tlv_buffer_size, TLV_DATA_128, &tlv_data_len);
        if (!tlv_value)
        {
